...
\begin{enumerate}
  ...
  % \item  For time covered by all segments $[t_i, t_i']$, we divide it into partitions $T_1, T_2,
    % \dots, T_p$ such that $T_i \cap T_j = \emptyset$ and $\cup_{i} T_i = \cup_i [t_i, t_i']$, also
    % when we get the partitions, if $T_i$ contains the start point of a job $a_i$ or a deadline  
...
jobs can all be completed, suppose there is an edge with capacity $T$ between $v_j$ and $u_p$, and
there is also an edge between $u_p$ and $g_i$ with capacity $T$,  
we know at time interval $T$ that job $v_j$ can be assigned to processor $g_i$. Afterwards we delete these
...
\begin{enumerate}
  ...
  \item We only add edge between job $v_j$ and the time $T_p$ where $a_j \leq A_p$ and $d_j >
    ...
    Also since we only add edge between $u_p$ and $g_i$ when processor $i$ is available at time $T_p$,
...
$O(|V|) + O(|E|)$, where $|V| = O(n+k), |E| = O(nk(n+k))$. So the time complexity of constructing
graph is $O(nk(n+k))$. The time complexity to find maximum flow with \textit{Edmonds-Karp} algorithm
is $O(|V||E|^2)$. So the time complexity of the proposed algorithm is $O(|V||E|^2)$, where  
$|V| = O(n+k), |E| = O(nk(n+k))$.
